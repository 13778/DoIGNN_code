...
def re_standard_transform(data: torch.Tensor, **kwargs) -> torch.Tensor:
    ...
    if not torch.is_tensor(mean):
        ...
        mean = mean.to(dtype=data.dtype, device=data.device)

    if not torch.is_tensor(std):
        std = torch.tensor(std, dtype=data.dtype, device=data.device)
    else:
        std = std.to(dtype=data.dtype, device=data.device)

    # ===== reshape：支持 N 或标量 =====
    mean = mean.reshape(-1).view(1, 1, -1)
    std  = std.reshape(-1).view(1, 1, -1)
