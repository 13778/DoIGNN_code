...
from ..metrics import masked_mae, masked_mape, masked_rmse, masked_kge, masked_nse
...
class BaseTimeSeriesForecastingRunner(BaseRunner):
    ...
    def __init__(self, cfg: dict):
        ...
        # define metric
        self.metrics = {"MAE": masked_mae, "RMSE": masked_rmse, "MAPE": masked_mape, "NSE": masked_nse, "KGE": masked_kge}
        ...
        if self.cl_param is not None:
            ...
            self.cl_step_size = cfg.TRAIN.CL.get("DoIGNN_SIZE", 1)
        # evaluation horizon
    ...
    def train_iters(self, epoch: int, iter_index: int, data: Union[torch.Tensor, Tuple]) -> torch.Tensor:
        ...
        # re-scale data
        prediction_rescaled = SCALER_REGISTRY.get(self.scaler["func"])(torch.squeeze(forward_return[0]), **self.scaler["args"])
        real_value_rescaled = SCALER_REGISTRY.get(self.scaler["func"])(torch.squeeze(forward_return[1]), **self.scaler["args"])
        # loss
        if self.cl_param:
            cl_length = self.curriculum_learning(epoch=epoch)
            if len(prediction_rescaled.shape) == 4:
    ...
    def test(self):
        ...
        for _, data in enumerate(self.test_data_loader):
            ...
            prediction.append(torch.squeeze(forward_return[0]))        # preds = forward_return[0]
            real_value.append(torch.squeeze(forward_return[1]))        # testy = forward_return[1]
