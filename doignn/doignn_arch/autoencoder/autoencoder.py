...
class AUTOENCODER(nn.Module):
    """An efficient unsupervised pre-training model for Time Series based on transFormer blocks. (AUTOENCODER)"""
    ...
    def encoding(self, long_term_history, mask=True):
        """Encoding process of AUTOENCODER: patchify, positional encoding, mask, Transformer layers.

        Args:
            long_term_history (torch.Tensor): Very long-term historical MTS with shape [B, N, 1, P * L],
                                                which is used in the AUTOENCODER.
    ...
    def decoding(self, hidden_states_unmasked, masked_token_index):
        """Decoding process of AUTOENCODER: encoder 2 decoder layer, add mask tokens, Transformer layers, predict.
    ...
    def forward(self, history_data: torch.Tensor, future_data: torch.Tensor = None, batch_seen: int = None, epoch: int = None, **kwargs) -> torch.Tensor:
        """feed forward of the AUTOENCODER.
            AUTOENCODER has two modes: the pre-training mode and the forecasting mode,
        ...
        Returns:
            ...
            forecasting:
                torch.Tensor: the output of AUTOENCODER of the encoder with shape [B, N, L, 1].
        """
        # reshape
