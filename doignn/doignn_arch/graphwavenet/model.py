...
class GraphWaveNet(nn.Module):
    def __init__(self, num_nodes, support_len, dropout=0.3, gcn_bool=True, addaptadj=True, aptinit=None, in_dim=2, out_dim=12, residual_channels=32, dilation_channels=32, skip_channels=256, end_channels=512, kernel_size=2, blocks=4, layers=2, if_prior=True, **kwargs):
        """
        Args:
            if_prior (bool): Whether to use prior graph in supports. Default: True
        """
        super().__init__()
        self.dropout = dropout
        self.blocks = blocks
        ...
        self.if_prior = if_prior
        self.A_prior = load_pkl("homozone/A_prior_homozone.pkl")
        self.A_prior = torch.from_numpy(self.A_prior).float()
    ...
    def forward(self, input, hidden_states, sampled_adj):
        ...
        Args:
            ...
            His (torch.Tensor): the output of AUTOENCODER of the last patch (segment) with shape [B, N, d].
        ...
        if in_len<self.receptive_field:
            ...
            x = input
        x = self.start_conv(x) #size for x: [16, 2, 130, 13]
        skip = 0

        #
        # ====== if use learned adjacency matrix, then reset the self.supports ===== #
        B = sampled_adj.size(0)
        A_prior_batched = self.A_prior.to(device=sampled_adj.device, dtype=sampled_adj.dtype) \
                              .unsqueeze(0).expand(B, -1, -1)
        
        if self.if_prior:
            self.supports = [
                self._calculate_random_walk_matrix(sampled_adj), 
                self._calculate_random_walk_matrix(sampled_adj.transpose(-1, -2)),
                self._calculate_random_walk_matrix(A_prior_batched), 
                self._calculate_random_walk_matrix(A_prior_batched.transpose(-1, -2))
            ]
        else:
            self.supports = [
                self._calculate_random_walk_matrix(sampled_adj), 
