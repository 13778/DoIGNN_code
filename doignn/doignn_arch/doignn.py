...
class DoIGNN(nn.Module):
    """Pre-training Enhanced Spatial-temporal Graph Neural Network for Multivariate Time Series Forecasting"""

    def __init__(self, dataset_name, pre_trained_autoencoder_path, autoencoder_args, backend_args, dgl_args):
        super().__init__()
        self.dataset_name = dataset_name
        self.pre_trained_autoencoder_path = pre_trained_autoencoder_path

        # iniitalize the autoencoder and backend models
    ...
    def load_pre_trained_model(self):
        ...
        # load parameters
        checkpoint_dict = torch.load(self.pre_trained_autoencoder_path)
        self.autoencoder.load_state_dict(checkpoint_dict["model_state_dict"])
        # freeze parameters
    ...
    def forward(self, history_data: torch.Tensor, long_history_data: torch.Tensor, future_data: torch.Tensor, batch_seen: int, epoch: int, **kwargs) -> torch.Tensor:
        ...
        # discrete graph learning & feed forward of AUTOENCODER
        bernoulli_unnorm, hidden_states, adj_knn, sampled_adj = self.discrete_graph_learning(long_term_history, self.autoencoder)
